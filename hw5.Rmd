...
### TASK 2
In the get_sheetz, we obtained the urls we needed for scraping that corresponded to the nodes for all 10 regions. After we get the urls, we wrote a function to get data: we read the html first, found out that the text part is some json data. Therefore, we used `fromJSON()` to get the data frame. One variable, `storeAttribute`, is a list of four data frames, in which one of four data frames is just the same as storeNumber (which already existed) so we only included the other 3 data frames. We saved the large list of 10 data frames as get_sheetz.rds.

In parse_sheetz, we first mapped the large list into a data frame of lists. Thereafter, we modified the data frame to change all NULLs to NAs (in order to quickly vectorize the data thereafter without losing observations), by using a helper function we created called `replace_null()`. Thereafter, we changed all of the columns that could be quickly vectorized into vectors of the appropriate type, based on the observation with the most elements. Finally, we removed any row that had an NA store number (indicating that it is an empty observation). 
...
sheetz <- tibble(lat=sheetz$latitude, 
                 lon=sheetz$longitude,
                 store_id=sheetz$storeNumber,
                 store_type="sheetz")
data <- bind_rows(wawa2, sheetz)
```
